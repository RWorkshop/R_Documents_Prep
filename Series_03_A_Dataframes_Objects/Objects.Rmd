...
These commands can be used to determine particular attributes.
    ...
    * names() - can find the labels of the individual columns.
    * dim() - can find the dimensions of a data object.
...
* The names of columns can be found by using the names()
command.
\begin{verbatim}
> names(ChickWeight)
...
* The columns can now be studied in the same ways as basic
vectors.
\begin{verbatim}
...
* Often, we would wish to make a subset of a larger data
...
\end{verbatim}
* It doesn't make sense to subset by columns. Therefore, the
